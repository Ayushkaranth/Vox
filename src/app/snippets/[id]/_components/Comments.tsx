...
function Comments({ snippetId }: { snippetId: Id<"snippets"> }) {
  ...
  const comments = useQuery(api.functions.snippets.getComments, { snippetId }) || [];
  const addComment = useMutation(api.functions.snippets.addComment);
  const deleteComment = useMutation(api.functions.snippets.deleteComment);
