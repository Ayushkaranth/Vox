...
function StarButton({ snippetId }: { snippetId: Id<"snippets"> }) {
  ...
  const isStarred = useQuery(api.functions.snippets.isSnippetStarred, { snippetId });
  const starCount = useQuery(api.functions.snippets.getSnippetStarCount, { snippetId });
  const star = useMutation(api.functions.snippets.starSnippet);
